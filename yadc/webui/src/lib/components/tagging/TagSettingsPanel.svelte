<script lang="ts">
    ...
    let isBatchTagging = $derived.by(() => {
        const s = $taggingStatuses.get(_datasetName);
    ...
    async function handleStart() {
        try {
            await startBatchTagging(_datasetName);
        } catch (e) {
    ...
    async function handleStop() {
        isCancelling = true;
        try {
            await stopTagging(_datasetName);
        } finally {
            isCancelling = false;
        }
    }
    ...
    <div class="flex-1 space-y-5 overflow-y-auto p-4">
        ...
        <section class="space-y-2">
            ...
            <p class="text-xs text-gray-500">
                ...
                position) = server default, shown as the placeholder. Rating is categorical — keep
                it low.
            </p>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">
                        WD-tagger tags ship underscored (e.g. <code class="text-gray-400"
                            >long_hair</code
                        >). On, they read as <code class="text-gray-400">long hair</code> in results
                        and saves. Kaomojis (e.g. <code class="text-gray-400">^_^</code>) are always
                        preserved.
                    </p>
                </div>
            </div>
        ...
        <section class="space-y-2">
            ...
            <p class="text-xs text-gray-500">
                Where to write each image's tags during the batch run.
            </p>
            <div class="space-y-3">
                <div>
                    ...
                    {#if saveMode === 'none'}
                        <p class="mt-2 text-xs text-gray-500">
                            Writes nothing. Results live only in each image's <strong>Tags</strong>
                            tab — held in memory, so they're lost on reload. Useful for auditing confidence
                            before committing to a save mode.
                        </p>
                    {:else if saveMode === 'draft'}
                        <p class="mt-2 text-xs text-gray-500">
                            Writes formatted tag text to a draft sidecar file per image (e.g. <code
                                class="text-gray-400"
                                >&lt;id&gt;.{assembledSave.draft_name}.draft~</code
                            >). Used as the training caption for image-generation fine-tuning.
                        </p>
                    {:else}
                        <p class="mt-2 text-xs text-gray-500">
                            Merges a <code class="text-gray-400">[tags]</code> section into each image's
                            TOML, preserving other keys.
                        </p>
                    {/if}
                </div>
                {#if saveMode !== 'none'}
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                ...
                                'draft'
                                    ...
                                    : '[tags] section in TOML'} are skipped entirely — no tag, no write.
                                Turn on to re-tag and rewrite every image.
                            </p>
                        </div>
                    </div>
                {/if}
                {#if saveMode === 'draft'}
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="label mb-1 block" for="tag-draft-name-batch"
                                >Draft name</label
                            >
                            <input
                        ...
                        <div>
                            <label class="label mb-1 block" for="tag-draft-format-batch"
                                >Format</label
                            >
                            <select
