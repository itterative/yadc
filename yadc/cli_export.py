...
@click.command(
    ...
    help='Export caption results to formats compatible with training tools. '
         'Backends: ' + ', '.join(f'{name}' for name in _BACKEND_NAMES) + '.',
)
@click.argument('dataset', type=click.File('r'))
@click.option(
    '--backend', '-b', type=click.Choice(_BACKEND_NAMES), default='sd-scripts',
...
@click.option(
    '--format', 'fmt', type=str, default=None,
...
@click.option(
    '--append/--overwrite', is_flag=True, default=False,
    help='Append to existing output instead of overwriting.',
)
@click.option(
    '--caption-extension', type=str, default='.txt',
...
def export(
    ...
    if fmt not in backend_desc.formats:
        _logger.error(
            ...
            backend_name, fmt, ', '.join(backend_desc.formats),
        )
        sys.exit(cmd_status.STATUS_USER_ERROR)
    ...
    _logger.info(
        ...
        len(resolved_images), backend_name, source, fmt,
    )
