...
def service():
    ...
    svc = ConfigHistoryService.__new__(ConfigHistoryService)
    svc._repo = mock_repo
...
class TestListHistory:
    ...
    def test_first_page_passes_unbounded_cursor(self, service):
        ...
        page = svc.list_history("alpha", limit=10)

        mock_repo.list_history.assert_called_once()
        _, kwargs = mock_repo.list_history.call_args
    ...
    def test_next_token_decoded_to_before_id(self, service):
        ...
        svc, mock_repo = service
        svc.list_history("alpha", limit=10, next="42")
    ...
    def test_next_token_emitted_when_more_pages(self, service):
        ...
        mock_repo.list_history.return_value = rows

        page = svc.list_history("alpha", limit=5)
    ...
    def test_empty_token_means_first_page(self, service):
        ...
        svc, mock_repo = service
        svc.list_history("alpha", limit=10, next="")
    ...
    def test_returns_config_history_page(self, service):
        ...
        svc, mock_repo = service
        mock_repo.list_history.return_value = [FakeEntry(id=1)]
