...
- `events.py` — `Event` base class (has `TYPE: ClassVar[str]`) and `SSEEvent` marker base (events broadcast over SSE subclass it, so `SSEEvents` can subscribe with a single handler), `SetupAppEvent` (with `app: Quart`), `StartupEvent`, `ShutdownEvent`, `PingEvent`, `CaptioningStatusEvent`, `DatasetChangedEvent`, `ResumptionFailedEvent`, `EnvironmentsChangedEvent` (with `envs: list[str]`), `TemplatesChangedEvent` (with `templates: list[str]`), `ImageCaptionedEvent` (with `caption: str`)
- `EventDispatcher` — `subscribe(event_cls, handler)`, `dispatch(event)`, `register_service(service)` (auto-scans for `@event_handler` methods), `@event_handler` decorator. Subscriptions are keyed by class and `dispatch` walks the event's MRO, so a handler subscribed to a base class (e.g. `SSEEvent`) receives every subclass
