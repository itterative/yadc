...
yadc/api/
  ...
  services/           # Business logic + repositories (both auto-discovered as Service subclasses)
    __init__.py           # re-exports all services and repositories (CaptioningService, TaggingService, TagSuggestionsService, DatasetJobService/JobKind/JobClaim/DatasetBusyError, DatasetLoader, DatasetScanner, DatasetService, DatasetRepository, DatasetUploadService/Result/UploadProgressEvent, ConfigHistoryService/Repository, ManagedDatasetsService, PromptGenerationService + PromptHistoryService + repos, SettingsService/Repository)
    ...
    tag_suggestions/      # TagSuggestionsService — autocomplete for the Tags tab's custom-tag input (`GET /tagging/suggest`). Pure-function split: `catalog.py` (data layer — BetaDoggo danbooru-tag-list download/parse, `TagCatalog` with per-canonical char→position jump lists + an inverted char index), `suggestions.py` (the pure `suggest(query, catalog, *, limit)` matcher — inverted-index candidate filter → jump-list subsequence pre-check → fzf-style fuzzy score (`yadc/utils/fuzzy.py`) × `log(post_count)` popularity boost), `service.py` (DI `Service` — lazy single-flight load + fire-and-forget startup preload; no process-wide state in the helpers). Currently pinned to the NoobAIXL variant; variants are selectable later via `CatalogVariant`. See `tagger-architecture` for the endpoint + matching pipeline.
      __init__.py           # Package re-export (TagSuggestionsService, TagCatalog, CatalogVariant, suggest, parse/cache helpers).
      catalog.py            # `TagCatalog`, `CatalogVariant` + `VARIANT_URL`, `load_catalog`/`download_catalog`/`parse_betadoggo_csv`, `normalize_tag_name`, cache-path helpers. Frozen dataclass with parallel `normalized`/`char_indices`/`counts`/`normalized_aliases` tuples; `char_index` is the inverted char → tag-index map over the union of canonical + alias chars.
      suggestions.py        # `suggest(query, catalog, *, limit)` — pure async matcher (yields every ~5k iterations). Returns `(tag, category)` tuples sorted by score desc, alpha tiebreak.
      service.py            # `TagSuggestionsService(Service)` — owns the parsed catalog + an `asyncio.Lock`; `@event_handler(StartupEvent)` warms it in the background so the first keystroke is fast.
    tagging.py             # TaggingService — ONNX tagger subprocess lifecycle (lazy spawn / idle teardown), result cache (bytes-aware `MemoryLRU`), single-image tag, batch tagging jobs, interactive save, cancel (graceful→kill). See `tagger-architecture` for the full subsystem.
```
