...
| `tagger_response_timeout_seconds` | `120.0` | Give up on a wedged-but-alive worker after this many seconds during a single **inference** (`tag()`). The startup/load phase is bounded separately by `tagger_startup_timeout_seconds`. |
| `tagger_startup_timeout_seconds` | `900.0` | Bound on the **startup** phase (`start()` → `load_model`), kept separate from the per-tag inference timeout because the first selection of a HuggingFace tagger downloads the model — a multi-hundred-MB fetch on a slow link can dwarf one inference. A wedged worker is still caught well before this via the liveness poll. |
