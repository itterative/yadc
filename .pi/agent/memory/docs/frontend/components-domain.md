...
```
  prompts/                        # Prompt-generator domain — feature folder
    PromptGenerator.svelte        # Host: two-column layout (form on left, streaming preview on right). Owns the form bindings + the Save-as-template dialog. Generate/Refine/History mode switcher at the top of the form column via `PillTabs` (three tabs, the form is rendered in the Generate and Refine tab children so state is shared via bindings; the History tab shows `PromptHistoryPanel`). Owns the `templateContent` local state for refine mode (ephemeral, not persisted). Two-piece form-tab state: `mode` (persisted `PromptGenMode`, drives the form / button label / `canGenerate` check) and `activeTab` (UI-only `generate` | `refine` | `history`, drives the PillTabs — initialised from `mode` via `untrack(() => mode)` so a returning user lands on the same Generate/Refine tab they left). The `Save to history` button in the form footer posts the current form state to `/api/prompts/history`; restore from the history panel switches `activeTab` + `mode` and populates `intent` / `focus` / `examples` / `templateContent` from the entry.
    PromptForm.svelte             # Env selector (with auto model fetch), model picker, intent textarea, focus toggle, examples section, Generate/Refine button. In refine mode (`mode === 'refine'`), renders a Template section between Environment and Intent (template picker `<select>` over `$templates.items` + `JinjaEditor` + variables chip strip) and relabels the intent field ("Refinement intent"). Mirrors `caption/EnvSelector` but bindable + reacts to streaming status. Form is re-mounted on tab switch (both tabs render it); internal state (loading flags, template picker selection) is per-instance, but `bind:value` against the host keeps all shared state in sync.
