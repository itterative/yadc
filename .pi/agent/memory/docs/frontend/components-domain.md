...
```
  prompts/                        # Prompt-generator domain — feature folder
    PromptGenerator.svelte        # Host: two-column layout (form on left, streaming preview on right). Owns the form bindings + the Save-as-template dialog. Generate/Refine mode switcher at the top of the form column via `PillTabs` (two tabs, the form is rendered in BOTH tab children so state is shared via bindings). Owns the `templateContent` local state for refine mode (ephemeral, not persisted).
