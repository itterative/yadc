...
```
  prompts/                        # Prompt-generator domain — feature folder
    PromptGenerator.svelte        # Host: two-column layout (form on left, streaming preview on right). Owns the form bindings + the Save-as-template dialog. `PillTabs` with two tabs: Compose (a single `PromptForm` instance) and History (`PromptHistoryPanel`). The form is rendered ONCE (not duplicated per tab), so env / model / intent / examples / template state all live in one place and never reset on a mode switch. Owns the `templateContent` local state for refine mode (ephemeral, not persisted). `mode` (persisted `PromptGenMode`, drives the footer button label / `canGenerate` / `canSave`) is bound to the form's inline New/Refine pills via `bind:mode`; `activeTab` (`compose` | `history`, UI-only) just switches between the form and the history panel. The `Save to history` button in the form footer posts the current form state to `/api/prompts/history`; restore from the history panel switches to the Compose tab and populates `mode` / `intent` / `focus` / `examples` / `templateContent` from the entry.
    PromptForm.svelte             # Env selector (with auto model fetch), model picker, intent textarea, focus toggle, mode toggle (New/Refine pills, bound to host `mode` via `bind:mode`), template section (refine only), examples section. Selecting Refine reveals the Template section (picker `<select>` over `$templates.items` + `JinjaEditor` + variables chip strip) between the mode pills and the examples, and relabels the intent field ("Refinement intent"). Mirrors `caption/EnvSelector` but bindable + reacts to streaming status. Rendered once by the host, so internal state (loading flags, template picker selection) survives a mode switch.
