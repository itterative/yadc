...
yadc/webui/src/lib/components/dataset/
  ...
  detail/                       # Image detail (tab host + tabs)
    ...
    Tags.svelte                 # Interactive tag tab — Tag button (sync `tagImage`) → result grouped by category (rating/general/character) as toggle chips with confidence % (prune by clicking off) → save bar (mode draft/extras + format/name) → interactive `POST .../images/<id>/tags`. Reads `tagResults`/`currentlyTagging`/`taggerStatus` stores; `onTagsSaved` refreshes caption/history so Caption/Extras tabs reflect the write. Owns `customTags` (SvelteMap), `orderedCategories` derived (pre-scores chips — custom 1.0, model from `result.tags`), and Save's `buildPrunedResult` integration (custom tags layered at synthetic 1.0 score, indistinguishable from a high-confidence model tag).
    TagCategoryChips.svelte     # One category's chip grid (header + Select all / Clear + chips + per-category `TagInput`). Pre-scored chip view (`TagChipView` exported type) is computed by the parent so this component only renders. Custom vs model color split, on/off line-through state. Custom chips carry an inline × button (`SvgClose`) that drops the tag entirely; the chip body still toggles on/off, so users get both "hide temporarily" and "remove entirely". The `TagInput` is shown for every category except `rating` (a static deny-list — ratings are categorical metadata, not free-form tags).
    TagInput.svelte             # Inline "+ custom" chip → text input for adding a tag not in the model output; sits at the end of the chip row. Submits via Enter (stays open for repeat adds); Esc/blur-with-empty closes. Also hosts the autocomplete dropdown (see the `GET /tagging/suggest` endpoint in `tagger-architecture`): recent tags (localStorage, via `recentTags` store) when the query is empty, catalog hits when typing (debounced 250ms, in-flight requests aborted on each keystroke and linked to the parent abort scope via `getAbortContext`/`linkedController`). Suggestion names mirror the tagger's `replace_underscores` setting (`$tagSettings.replaceUnderscores` is passed to `fetchTagSuggestions`), so picked tags commit in the same form as the model output. The list is a `popover="auto"` top-layer element with manual `position: fixed` placement that follows the input on scroll/resize/reflow and flips/clamps into the viewport; feature-detects the popover API and skips the dropdown entirely (falling back to manual text input only) on browsers without it. ↑/↓ navigates, Enter commits the highlighted row, Esc dismisses (or closes the input from recent). Standalone component so future flows (e.g. multi-tag paste) can replace the input without touching Tags.svelte.
