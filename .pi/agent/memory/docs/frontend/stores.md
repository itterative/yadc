...
yadc/webui/src/lib/stores/
  ...
  prompts/                       # Prompt-generator domain — streaming generation
    ...
    settings.ts                  # Persisted form settings (env, apiUrl, apiToken, apiModelName, intent, focus) backed by localStorage via the shared `storable` utility. `promptSettings` is a Svelte writable store; `resetPromptSettings()` resets to defaults (also fires through the store, so it gets persisted). `examples` are NOT persisted (image data URLs are too large; no re-fetchable source per example yet — see plan history for the deferred IndexedDB follow-up). Storage key: `yadc/prompts/formSettings` (`$version: 1`). Follows the same shape as `stores/settings.ts` and `stores/caption/settings.ts`. The host component uses the controlled-component pattern (local `$state` initialized from `$promptSettings`, single `$effect` syncing back via `promptSettings.update`) — `bind:prop={$store.field}` does NOT work reliably in Svelte 5 (the `$store` auto-subscription is read-only).
