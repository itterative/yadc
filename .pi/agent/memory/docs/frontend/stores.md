...
yadc/webui/src/lib/stores/
  ...
  prompts/                       # Prompt-generator domain — streaming generation
    ...
    types.ts                     # ExamplePair + PromptGenFocus + PromptGenRequest + StreamEvent + GenerationStatus
    api.ts                       # `generatePrompt()` — POST to /api/prompts/generate, iterate NDJSON stream via ReadableStream reader, dispatch token/reasoning/done/error callbacks. `fetchImageAsDataUrl()` (browser fetch + FileReader) and `readFileAsDataUrl()` (local File → data URL) for the multimodal examples panel.
    ...
    actions.ts                   # `startGeneration()` — wires the streaming callbacks to the state mutators and surfaces errors via toasts + the generation state.
    settings.ts                  # Persisted form settings (mode, env, apiUrl, apiToken, apiModelName, intent, focus) backed by localStorage via the shared `storable` utility. `promptSettings` is a Svelte writable store; `resetPromptSettings()` resets to defaults (also fires through the store, so it gets persisted). `examples` are NOT persisted (image data URLs are too large; no re-fetchable source per example yet — see plan history for the deferred IndexedDB follow-up). Storage key: `yadc/prompts/formSettings` (`$version: 2` after Phase 6b; the v1→v2 `migrateToV2` helper re-adds the `mode: 'generate'` field on top of v1 objects so existing users keep their env / apiUrl / intent / focus). `mode` is persisted so the user comes back to the same Generate/Refine tab; `templateContent` and the template picker selection are NOT persisted (ephemeral local state — the user re-selects a template to re-fetch on reload). Follows the same shape as `stores/settings.ts` and `stores/caption/settings.ts`. The host component uses the controlled-component pattern (local `$state` initialized from `$promptSettings`, single `$effect` syncing back via `promptSettings.update`) — `bind:prop={$store.field}` does NOT work reliably in Svelte 5 (the `$store` auto-subscription is read-only). First user of the `storable` `migrate` argument pattern (see `storable.js`); the bug fix in `storable.js` that actually writes the migrated data back to the store was necessary for the migration to take effect.
