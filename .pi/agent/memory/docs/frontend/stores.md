...
yadc/webui/src/lib/stores/
  ...
  tagging/                       # Tagger domain — mirrors caption/ (role-based). Backed by the tagger subsystem (see `docs/tagger-architecture.md`).
    ...
    types.ts                     # TaggerResult / TagCustomizations / TagJobInfo / TagSaveOptions / TaggerStatus + TagSuggestion (autocomplete row) + SuggestionVariantResponse/SuggestionVariantOption (variant picker payload) + the 4 SSE event shapes + DEFAULT_SAVE
    api.ts                       # tagImage (sync single-image), startTagJob / stopTagJob / fetchTagJobStatus (batch job), saveImageTags (interactive prune save), fetchTagSuggestions (autocomplete, with a session-scoped LRU keyed by normalized query+limit+replace_underscores so a setting flip re-fetches the matching form), fetchSuggestionVariant/setSuggestionVariant (GET/PUT catalog variant — no busy/in-progress states, throws on non-200). No `withPasswordRetry` — tagger endpoints don't require a password. `previewImageTags` (image-scoped preview) doubles as the per-image selection persistence path: it forwards threshold query params (to land in the same cache bucket as POST /tag) and an optional `customizations` payload that the backend writes onto the cached `TaggerResult` so navigation restores the prune/`customTags` state.
    ...
    settings.ts                  # tagSettings storable (`yadc/tagSettings`, $version 3): threshold overrides (null=server default) + saveMode/draftName/draftFormat. CANONICAL_THRESHOLDS constant (wd-tagger 0.0/0.35/0.85) is the diff-dot baseline.
    ...
    highlights.ts                # tagHighlights storable (`yadc/tagHighlights`, $version 1) — global curated tag tiers (starred/desired/undesired) + `categoryOverrides` (starred tag → forced section) , matched by name across datasets. tagTierMap derived (tag→tier lookup), tagCategoryOverrideMap derived (tag→section). Helpers: setTagTier/removeTagTier/clearTier (mutual-exclusion; also drops a departing tag's override) and setTagCategoryOverride/removeTagCategoryOverride.
    policy.ts                    # tagPolicy storable (`yadc/tagPolicy`, $version 1) — always-add / banned lists separate from the highlight tiers (the Customize tab's starred/undesired chips act as quick-add shortcuts for these, not as the lists themselves). Helpers: addToAlwaysAdd / removeFromAlwaysAdd / clearAlwaysAdd / addToBanned / removeFromBanned / clearBanned. snapshotTagPolicy() returns a wire subset (alwaysAdd/banned) the action layer sends with each tagging request; the read is synchronous so the caller doesn't need to be inside an effect. alwaysAddList / bannedList derived for chip-row bindings. The policy is a backend read-time transform (`apply_policy` in `yadc/taggers/postprocessing.py`), applied over the cached value — not part of the cache key — so toggling a policy tag shows up on the next read without re-tagging.
    selectionModel.ts           # Pure selection-model logic for the per-image prune grid (no Svelte reactivity — unit-testable). `starredSection` is the single source of truth for where a starred tag files (override → character/general, else model category → character, else general). `computeOrderedCategories` builds the section/chip layout (starred pulled to lead, absent starred force-shown). `computeSelection` builds the saved pruned result + persisted customizations snapshot in one pass. Also defines `TagChipView`, `OrderedCategory`, `SECTION_ORDER`, `pushBucket`.
