...
- **DatasetImage persistence**: `.txt` for caption, `.toml` for metadata extras, `.history~` for versioned history (TOML entries separated by `----------` markers), `.<name>.draft~` for named drafts. History is saved on every caption update (both captioning jobs and manual webui edits) and extras updates. History entries can be browsed (`GET /images/<id>/history`), restored (`PUT /images/<id>/history/<index>/restore`), and deleted (`DELETE /images/<id>/history/<hash>` — uses content hash for safe identification). Drafts can be deleted via `DELETE /images/<id>/drafts/<name>`.
...
- **Append + staging flow**: `POST /datasets/<name>/upload` starts an append upload (writes files to a temporary `.staging/<uuid>/` dir). `POST /datasets/<name>/staging/commit` resolves user-chosen conflicts and moves the staged files into the live managed layout. Constants: `STAGING_DIR_NAME = ".staging"`, `STAGING_MAX_AGE_SECONDS = 24 * 3600`. A periodic `JobScheduler` job in `DatasetUploadService.__init__` cleans up stale staging directories.
- **Upload source-id propagation**: All three upload endpoints (create, append, commit) accept an optional `?source=<clientId>` query parameter, threaded through the service as a kwarg. The service calls `DatasetWatcherService.expect_file_change(name, str(dest_path), source=source)` before each file write (create) and each `shutil.move` from staging (commit), and passes `source=source` to the final `rescan_dataset(...)`. The result is that the originating tab's `DatasetChangedEvent` carries its own `clientId` and the SSE listener in `events.ts` suppresses it — without this, a drop-and-upload would show a spurious "files have changed" refresh banner to the user who just initiated the upload. See `docs/dataset-watcher` for the full pattern.
...
- **Dataset rescan**: `DatasetService._apply_disk_scan` now does a **diff scan** — compares the freshly-walked disk state against the current index and only writes SQL for changed rows. Returns `bool` (whether anything changed). Both `_refresh_stale_datasets` (background job, scheduled via `JobScheduler` at `dataset_refresh_interval_seconds` interval, default 300s) and `rescan_dataset(source=...)` dispatch a `DatasetChangedEvent` only when the scan found real changes — avoiding spurious SSE notifications. The background job and any future manual refresh are serialized via `DatasetService._refresh_lock` to prevent SQLite write-lock contention.
- **Background dataset refresh**: The `JobScheduler` is optional so tests can construct the service without it. The lock blocks concurrent refreshers on the SQLite write lock; inotify via the watcher still drives live updates in the common case — the stale-refresh job is a fallback for changes the watcher can't see (e.g. external edits to a dataset's `config.toml` that add new image paths, where no inotify event fires).
