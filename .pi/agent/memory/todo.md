...
## AbortController for focused item / preview navigation

Now that the side panel follows the lightbox preview (via `previewItem → focusedItem` effect in `routes/datasets/[name]/+page.svelte`), users can navigate between images much faster — arrow keys, double-click new image while panel is open, etc. This surfaces a race condition: in-flight `fetchCaption` / `fetchHistory` / `fetchPromptPreview` requests can resolve out of order and overwrite the latest data with stale data from a previous image.

`ImageDetail.svelte` already has a `cancelled` flag for the caption/history fetch, but it only guards the `.then()` callbacks — it doesn't actually abort the network request. `PromptPreview.svelte` has a similar debounced-fetch pattern with a `cancelled` flag.

**Plan**: plumb an `AbortController` (or `AbortSignal`) through to `fetchCaption`, `fetchHistory`, `fetchPromptPreview` (and any other request that depends on the focused item) so switching images cancels the in-flight request instead of just discarding the response. The `fetch*` helpers in `$lib/stores/dataset/api.ts` should accept an optional `signal` parameter and pass it to `fetch()`.

Key files: `yadc/webui/src/lib/stores/dataset/api.ts`, `yadc/webui/src/lib/components/dataset/detail/ImageDetail.svelte`, `yadc/webui/src/lib/components/dataset/detail/Preview.svelte` (which uses `PromptPreview.svelte`).
