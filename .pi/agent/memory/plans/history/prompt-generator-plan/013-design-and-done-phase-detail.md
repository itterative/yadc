...
date: 2026-06-21
---
# Design + done-phase detail (consolidated from the plan)

**Context:** By 2026-06-21 the prompt-generator plan had grown to
~1437 lines — it carried the original design record (decisions, a full
architecture sketch with code) plus detailed step lists and code
sketches for every phase, including the ones already shipped. The plan
was trimmed down to a current-state tracker (condensed architecture,
per-phase status, and the remaining work). This entry preserves
everything that was removed so nothing is lost.

**Decision / Rationale:** Consolidate the cross-cutting design record
and the done-phase overview into one history entry. The per-phase
*implementation* records (the gritty "what actually landed + the
corrections made during impl") already live in entries `001`, `002`,
`005`, `006`, `007`, `008`, `009`, `011`, `012`; this entry is their
*design-level* complement — the original decisions, the architecture
sketch, the planned phase shape, the resolved decisions, the open
questions as they stood, and the status / user-feedback notes captured
at trim time.

Below is the lifted content, in original document order, with short
notes marking where remaining-work sections (kept in the plan) were
skipped.
...
> *Skipped here: Phase 4 (CLI parity) and Phase 5 (optional polish) —*
> *both remain planned and are kept in the plan.*
...
> *Skipped here: the condensed **Key Files Touched** list — kept in the*
> *plan. The two sub-lists below were the plan's per-phase file notes*
> *(labelled "(planned)" but since implemented — see the Phase 5b / 6*
...
> *Skipped here: Phase 8 (meta-prompt Jinja templating) — proposed,*
> *awaiting a go/no-go; kept in the plan.*
