...
| [`prompt-generator-plan`](.pi/agent/memory/plans/prompt-generator-plan.md) | Meta-prompting feature — generate (or refine) high-quality Jinja2 templates from an intent (and optional few-shot examples) using any configured env's model, with streaming + cancel. Extracts a generic `BaseLLMClient` layer (`yadc/llm/`) from existing per-backend captioners. | In Progress (generation, refine mode, prompt-history persistence done; CLI command + meta-prompt tuning remain; meta-prompt Jinja templating proposed) |
