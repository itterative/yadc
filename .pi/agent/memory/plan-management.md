...
| [`api-request-consolidation-plan`](.pi/agent/memory/plans/api-request-consolidation-plan.md) | Consolidate redundant API requests — env list/details, dataset browser (4→1), SSE caption text | Not started |
| [`captioning-status-bar-plan`](.pi/agent/memory/plans/captioning-status-bar-plan.md) | Refine captioning progress UI — topbar spinner + absolute progress bar, side panel Start/Stop button swap. Phase 1 done, Phase 2 (store refactor) pending | In progress |
