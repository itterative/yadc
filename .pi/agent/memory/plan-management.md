...
| [`prompt-generator-plan`](.pi/agent/memory/plans/prompt-generator-plan.md) | Meta-prompting feature — generate high-quality Jinja2 templates from an intent (and optional few-shot examples) using any configured env's model, with streaming + cancel. Extracts a generic `BaseLLMClient` layer (`yadc/llm/`) from existing per-backend captioners. | Phase 1 + 2 complete; Phase 3 (frontend) next |
