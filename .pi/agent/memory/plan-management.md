...
| [`prompt-generator-plan`](.pi/agent/memory/plans/prompt-generator-plan.md) | Meta-prompting feature — generate (or refine) high-quality Jinja2 templates from an intent (and optional few-shot examples) using any configured env's model, with streaming + cancel. Extracts a generic `BaseLLMClient` layer (`yadc/llm/`) from existing per-backend captioners. | Phase 1 + 2 + 3 + 5b + 6a (refine-mode backend) complete; Phase 4 (CLI) + 5 (meta-prompt tuning) + 6b (refine-mode frontend) remain |
