...
| [`desktop-exe-plan`](.pi/agent/memory/plans/desktop-exe-plan.md) | Ship the yadc webui as a standalone Windows .exe via PyInstaller. Reuses `yadc webui serve` with a new `--browser` flag, auto-opens the SPA on launch, builds two variants (CPU + GPU). Adds a `scripts/build_windows_exe.{ps1,sh}` build driver and `docs/desktop-exe.md` user docs. | Complete |
...
| [`prompt-generator-plan`](.pi/agent/memory/plans/prompt-generator-plan.md) | Meta-prompting feature — generate/refine Jinja2 prompt templates from an intent (and optional few-shot examples) using any configured env's model, with streaming + cancel + server-side prompt history. Built on a generic `BaseLLMClient` layer (`yadc/llm/`) extracted from the per-backend captioners. | Mostly done (generation, refine, history, settings tab shipped); CLI parity + optional polish remain; meta-prompt Jinja templating proposed |
| [`tagger-plan`](.pi/agent/memory/plans/tagger-plan.md) | ONNX image-tagging feature — base abstraction, `OnnxTagger` for SmilingWolf/WD models, multiprocessing client/server, dataset-image API endpoint, CLI, batch tagging job + draft/extras saves, cancel subsystem, underscore replacement, scored formatter. Backend + WebUI surface both complete. | Complete (backend + WebUI; CSV import is a future iteration) |
| [`tagger-model-swap-plan`](.pi/agent/memory/plans/tagger-model-swap-plan.md) | User-configurable tagger model — let the user pick / swap the active model from the WebUI, drain in-flight work, persist via SQLite, ship SmilingWolf HF repos in v1. | Complete |
| [`dataset-job-coordinator-plan`](.pi/agent/memory/plans/dataset-job-coordinator-plan.md) | `DatasetJobService` — cross-service per-dataset job mutex so captioning and tagging (batch + single-image) can never overlap on the same dataset. Extensible to future sidecar-writing job kinds. | Complete |
