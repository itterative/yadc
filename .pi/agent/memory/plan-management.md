...
| [`cli-api-captioning-shared-runner-plan`](.pi/agent/memory/plans/cli-api-captioning-shared-runner-plan.md) | Extract `CaptioningRunner` core shared between CLI `caption` command and API `AsyncCaptionJob`. First step toward CLI/API unification. | Approved (pending implementation) |
